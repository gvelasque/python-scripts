class MyHashSet:
    ...
    def add(self, key: int) -> None:
        if not self.contains(key) and self.max_len_key(key):
            self.list_nums.append(key)

    def remove(self, key: int) -> None:
        if self.contains(key):
            self.list_nums.remove(key)

    def contains(self, key: int) -> bool:
        """
        Return true if this set contains the specified element.
        """
        return key in self.list_nums

    @staticmethod
    def max_len_key(key: int) -> bool:
        return 0 <= key <= (10 ** 6)
