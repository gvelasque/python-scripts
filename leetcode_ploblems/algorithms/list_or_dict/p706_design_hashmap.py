...
class MyHashMap:
    ...
    def put(self, key: int, value: int) -> None:
        ...
        """
        if self.max_key_value(key, value):
            self.dict_nums[key] = value

    def get(self, key: int) -> int:
        """
        Return the value to which the specified key is mapped, or -1 if this
        map contains no mapping for the key.
        """
        if key in self.dict_nums.keys():
            return self.dict_nums[key]
        else:
            return -1
    ...
    def remove(self, key: int) -> None:
        ...
        """
        if key in self.dict_nums.keys():
            del self.dict_nums[key]

    @staticmethod
    def max_key_value(key: int, value: int) -> bool:
        return 0 <= (key or value) <= (10 ** 6)
