...
def roman_to_int(roman: str) -> int:
    ...
    Constraints:
        - 1 <= roman.length <= 15
        - roman contains only the characters (
        'I', 'V', 'X', 'L', 'C', 'D', 'M')
        - It is guaranteed that roman is a valid roman numeral
        in the range [1, 3999].
    """
    ...
    nums = []
    for i in roman:
        nums.append(roman_list[i])

    total, index = 0, 0
    while len(nums) > index:
        print(nums[index])

        if (len(nums) - 1) > index and nums[index + 1] > nums[index]:
