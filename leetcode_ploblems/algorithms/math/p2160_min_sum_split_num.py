...
def min_sum(num: int) -> int:
    ...
    Example 1:
        - Input: num = 2932
        - Output: 52
        - Explanation: Some possible pairs [new1, new2] are [29, 23], [223,
          9], etc. The minimum sum can be obtained by the pair [29, 23]: 29
          + 23 = 52.
    ...
    Example 2:
        ...
        - Explanation: Some possible pairs [new1, new2] are [0, 49], [490, 0],
          etc. The minimum sum can be obtained by the pair [4, 9]: 4 + 9 = 13.

    Constraints:
        - 1000 <= num <= 9999
"""
    if num <= 1000 or num >= 9999:
        raise ValueError("The range has to be between 1000 and 9999")

    nums = [int(digit) for digit in str(num)]

    total = 0
    while len(nums) > 1:
        minimum = nums.pop(nums.index(min(nums)))
        maximum = nums.pop(nums.index(max(nums)))
        total += int(f"{str(minimum)}{str(maximum)}")
